...
import { getProduct } from '../api/django.js'
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        // تصاویر: ابتدا PocketBase، سپس بک‌اند
        const allImages = []
        try {
          const pbImgs = await getProductImages(id)
          pbImgs.forEach((r) => {
            const url = pbImageUrl(r, r.image)
            if (url) allImages.push(url)
            // features از PocketBase
            if (r.features) {
              try {
                const parsed = typeof r.features === 'string'
        ...
        // تصاویر بک‌اند (اگر موجود بود)
        if (prod?.images && Array.isArray(prod.images)) {
          prod.images.forEach((img) => {
            const url = typeof img === 'string' ? img : img?.image || img?.url
            if (url && !allImages.includes(url)) allImages.push(url)
          })
        } else if (prod?.image && !allImages.includes(prod.image)) {
          allImages.push(prod.image)
        }
